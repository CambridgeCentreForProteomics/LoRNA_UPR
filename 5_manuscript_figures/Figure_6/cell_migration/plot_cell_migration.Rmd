...
t.test(for_ttest_paired$Control, for_ttest_paired$eIF3d, paired=TRUE)
...
```
Repeating the cell migratiopn plots for the 3j siRNA data.
```{r}
...
all_assays_3j <- sheets_3j %>% lapply(function(sheet_n){
  ...
  to_plot <- cell_migration_data %>% pivot_longer(cols=-Time) %>%
    ...
    mutate(siRNA=recode(siRNA, '0'='Control', 'siCt'='Control', 'sieIF3d'='eIF3d')) %>%
