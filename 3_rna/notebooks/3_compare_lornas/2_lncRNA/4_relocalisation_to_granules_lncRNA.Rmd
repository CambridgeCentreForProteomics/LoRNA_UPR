...
sig <- to_plot %>%
  group_by(AU_bin) %>%
  do(broom::tidy(wilcox.test(formula=diff~gene_biotype, data=.))) %>%
...
```
Tabulate the number of RNAs in each category above.
```{r}
au_content_biotype_n <- to_plot %>% group_by(AU_bin, gene_biotype) %>% tally() %>%
  mutate(gene_biotype=paste0('n_', gene_biotype)) %>%
  pivot_wider(names_from=gene_biotype, values_from=n)

print(au_content_biotype_n)

au_content_biotype_n %>% merge(sig,by='AU_bin') %>%
  dplyr::select(-gene_biotype) %>%
  write.table(sep='\t',
              quote=FALSE,
              row.names=FALSE,
              file='../../../../5_manuscript_figures/Figure_5/lncRNA/lncRNA_vs_mRNA_au_content_granule_reloc_summary.tsv')
...
sig <- to_plot %>%
  group_by(transcript_length_bin) %>%
  do(broom::tidy(wilcox.test(formula=diff~gene_biotype, data=.))) %>%
  filter(p.value<0.001) %>%
  mutate(gene_biotype='mRNA')
...
transcript_length_biotype_n <- to_plot %>%
  group_by(transcript_length_bin, gene_biotype) %>% tally() %>%
  mutate(gene_biotype=paste0('n_', gene_biotype)) %>%
  pivot_wider(names_from=gene_biotype, values_from=n)

transcript_length_biotype_n %>% merge(sig,by='transcript_length_bin') %>%
  dplyr::select(-gene_biotype) %>%
  write.table(sep='\t',
              quote=FALSE,
              row.names=FALSE,
              file='../../../../5_manuscript_figures/Figure_5/lncRNA/lncRNA_vs_mRNA_length_granule_reloc_summary.tsv')
