...
p <- proportions_vs_MERFISH %>%
  ...
  theme(legend.position=c(1, 0.1), legend.justification=c(1, 0.1), legend.box=element_blank())
...
p <- proportions_vs_MERFISH %>%
  ...
  theme(legend.position=c(1, 0.05), legend.justification=c(1, 0.05), legend.box=element_blank())
...
print(p %+% (filter(proportions_vs_MERFISH, biotype.nuc %in% c('protein_coding', 'retained_intron'))) + facet_wrap(~biotype.nuc) + theme(strip.background=element_blank()))
```
