...
```{r}
length_ctrl_loc_n <- to_plot %>%
  filter(major_localisation %in% c('Cytosol', 'Membrane', 'Nucleus')) %>%
  group_by(binned_length, major_localisation) %>% tally() %>%
  mutate(major_localisation=paste0('n_', major_localisation)) %>%
  pivot_wider(names_from=major_localisation, values_from=n)

print(length_ctrl_loc_n)

length_ctrl_loc_n %>% merge(sig, by='binned_length') %>%
  write.table(sep='\t',
              quote=FALSE,
              row.names=FALSE,
              file='../../../../../5_manuscript_figures/Figure_5/features_granules/length_loc_summary.tsv')
